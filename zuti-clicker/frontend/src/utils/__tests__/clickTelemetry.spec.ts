...
describe("buildDigest", () => {
  it("produces a digest whose bucket sum matches clicks - 1", () => {
    ...
    const digest = buildDigest({
      ...
      integrityFlags: [],
      weakSignals: [],
      methodCounts: { primary: 5, secondary: 0, enter: 0, space: 0, touch: 0, other: 0 }
    });
  ...
  it("carries counters, flags, and methodCounts through unchanged", () => {
    const digest = buildDigest({
      ...
      methodCounts: { primary: 0, secondary: 1, enter: 0, space: 0, touch: 0, other: 0 }
    });
    ...
    expect(digest.methodCounts).toEqual({ primary: 0, secondary: 1, enter: 0, space: 0, touch: 0, other: 0 });
  });
});
